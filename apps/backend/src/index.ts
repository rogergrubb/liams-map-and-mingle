// Build: 2025-12-13 - Emergency contacts and safety routes v3.0
...
async function runSafetyMigration() {
  try {
    ...
    // User visibility spectrum columns
    console.log('🔄 Adding User visibility columns...');
    await prisma.$executeRawUnsafe(`ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "visibilityLevel" TEXT DEFAULT 'circles'`).catch(() => {});
    await prisma.$executeRawUnsafe(`ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "visibilityUpdatedAt" TIMESTAMP(3)`).catch(() => {});
    await prisma.$executeRawUnsafe(`ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "beaconExpiresAt" TIMESTAMP(3)`).catch(() => {});
    await prisma.$executeRawUnsafe(`ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "beaconDuration" INTEGER DEFAULT 60`).catch(() => {});
    await prisma.$executeRawUnsafe(`ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "locationPrecision" TEXT DEFAULT 'precise'`).catch(() => {});
    await prisma.$executeRawUnsafe(`ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "allowDiscovery" BOOLEAN DEFAULT true`).catch(() => {});
    await prisma.$executeRawUnsafe(`ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "showToCirclesOnly" BOOLEAN DEFAULT false`).catch(() => {});
    console.log('✅ User visibility columns added');
