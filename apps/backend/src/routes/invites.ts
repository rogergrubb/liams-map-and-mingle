...
invitesRoutes.post('/send', async (c) => {
  try {
    ...
    const customMessage = message || "I've been using Map & Mingle to connect with amazing people nearby. Join me and find your people too!";
    ...
    try {
      ...
      // Table might not exist, that's okay - continue without tracking
    ...
    if (email) {
      ...
      if (!resend) {
        ...
        return c.json({ 
          ...
          fallback: true,
          shareUrl: inviteUrl,
          message: 'Please copy and share this link instead: ' + inviteUrl
        }, 503);
      }

      try {
        // Use configured from email, or fall back to Resend's test domain
        // Note: For production, verify your domain at https://resend.com/domains
        let fromEmail = config.email.fromEmail;
        if (!fromEmail || fromEmail.includes('mapandmingle.com')) {
          // Resend requires verified domain - use their test domain for now
          fromEmail = 'Map & Mingle <onboarding@resend.dev>';
        }
        console.log(`Attempting to send invite email from ${fromEmail} to ${email}`);
          ...
          html: `
            ...
            <body style="margin: 0; padding: 0; background-color: #f3f4f6; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
              <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f3f4f6; padding: 40px 20px;">
                <tr>
                  <td align="center">
                    <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 500px; background: linear-gradient(135deg, #ec4899 0%, #8b5cf6 50%, #6366f1 100%); border-radius: 24px; overflow: hidden;">
                      <tr>
        ...
        console.log(`Invite email sent successfully to ${email}:`, result);
        return c.json({ success: true, message: 'Invite sent!', emailId: result.data?.id });
        ...
        return c.json({ 
          error: 'Failed to send email. Please try copying the link instead.',
    ...
    if (phone) {
      ...
      console.log(`SMS invite requested to ${phone} from ${userId} - Twilio not configured`);
      
      return c.json({ 
        success: true, 
        message: 'SMS invites coming soon! For now, please share the link directly.',
        shareUrl: inviteUrl,
        smsNotAvailable: true
      });
    }
...
// GET /api/invites/config - Check if invite services are configured
invitesRoutes.get('/config', async (c) => {
  return c.json({
    emailConfigured: !!resend,
    smsConfigured: false, // Twilio not yet implemented
    shareUrl: 'https://www.mapandmingle.com'
  });
});
