...
authRoutes.post('/register', rateLimitMiddleware(10, 15 * 60 * 1000), async (c) => {
  try {
    ...
    return c.json({ error: 'Registration failed', detail: (error as any)?.message || String(error) }, 500);
  }
});
...
authRoutes.post('/login', rateLimitMiddleware(10, 15 * 60 * 1000), async (c) => {
  try {
    ...
    return c.json({ error: 'Login failed', detail: (error as any)?.message || String(error) }, 500);
  }
});
    ...
    try {
      ...
      if (resendApiKey) {
        await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            from: 'Map & Mingle <noreply@mapandmingle.com>',
            to: email,
...
authRoutes.post('/reset-password', async (c) => {
  try {
    ...
    if (credentialAccount) {
      ...
      // Create new credential account for Google-only users
      // This allows them to login with email/password after setting a password
      await prisma.account.create({
        data: {
          userId: user.id,
          accountId: user.email, // Use email as accountId for credential accounts
          providerId: 'credential',
          password: hashedPassword,
        },
      });
...
authRoutes.get('/google/callback', async (c) => {
  ...
  try {
    ...
    const retryDbOperation = async <T>(operation: () => Promise<T>, retries = 3): Promise<T> => {
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          // Warm up connection with a simple query first
          if (attempt === 1) {
    ...
    try {
      // First try simple query without includes - with retry
