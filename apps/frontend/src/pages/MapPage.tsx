...
function MapController() {
  ...
  useEffect(() => {
    ...
    const handleMoveEnd = () => {
      ...
      }, 100); // Wait 100ms after last move before fetching
    };
    ...
    // Auto-refresh pins every 20 seconds (skip if already loading)
    let isPolling = false;
    const pollingInterval = setInterval(async () => {
      if (isPolling) return; // Skip if previous request still in progress
      isPolling = true;
      try {
        const bounds = map.getBounds();
        const zoom = map.getZoom();
