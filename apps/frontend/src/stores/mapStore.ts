...
import { handleLimitError } from './upgradeStore';
...
export const useMapStore = create<MapState>((set, get) => ({
  ...
  addPin: async (pinData) => {
    try {
      ...
      // Check if it's a limit error and show upgrade modal
      if (!handleLimitError(error, 'free')) {
        throw error;
      }
      throw error; // Still throw so UI knows it failed
    }
  },
}));
