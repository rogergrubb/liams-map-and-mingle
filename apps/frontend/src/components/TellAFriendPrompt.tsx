...
export default function TellAFriendPrompt({ onClose }: TellAFriendPromptProps) {
  ...
  const handleSendInvite = async () => {
    ...
    try {
      ...
      // Check if we got a fallback response (SMS not available or email failed)
      if (response.smsNotAvailable || response.fallback) {
        setSent(true);
        haptic.confirm();
        setEmail('');
        setPhone('');
        
        // If there's a fallback, copy the link automatically
        if (response.shareUrl) {
          try {
            await navigator.clipboard.writeText(response.shareUrl);
            setError(response.message || 'Link copied! Share it with your friend.');
          } catch {
            setError(response.message || 'Share this link: ' + response.shareUrl);
          }
        }
      } else {
        setSent(true);
      ...
      // If there's a fallback URL, show it
      if (errorData?.fallback && errorData?.shareUrl) {
        try {
          await navigator.clipboard.writeText(errorData.shareUrl);
          setError('Email unavailable - link copied! Share it manually.');
          setSent(true);
        } catch {
          setError(`Share this link: ${errorData.shareUrl}`);
        }
      } else {
        setError(errorData?.error || 'Failed to send invite. Try copying the link instead!');
      }
    } finally {
      setSending(false);
    }
