...
const modeConfig = {
  everybody: { icon: Globe, label: 'Everybody', shortLabel: 'Looking For', color: 'from-gray-500 to-gray-700' },
  dating: { icon: Heart, label: 'Dating', shortLabel: 'Dating', color: 'from-pink-400 to-rose-500' },
  friends: { icon: Users, label: 'Friends', shortLabel: 'Friends', color: 'from-purple-400 to-indigo-500' },
  networking: { icon: Briefcase, label: 'Networking', shortLabel: 'Network', color: 'from-blue-400 to-cyan-500' },
  events: { icon: Calendar, label: 'Events', shortLabel: 'Events', color: 'from-green-400 to-emerald-500' },
  travel: { icon: Plane, label: 'Travel', shortLabel: 'Travel', color: 'from-orange-400 to-amber-500' },
};

const visibilityConfig: Record<VisibilityLevel, { 
  icon: typeof Ghost; label: string; shortLabel: string; color: string; bgColor: string; description: string;
}> = {
  ghost: { icon: Ghost, label: 'Ghost Mode', shortLabel: 'Ghost', color: 'text-gray-600', bgColor: 'bg-gray-100 border-gray-300', description: 'Completely invisible' },
  circles: { icon: Shield, label: 'Circles Only', shortLabel: 'Circles', color: 'text-blue-600', bgColor: 'bg-blue-50 border-blue-200', description: 'Only trusted circles' },
  fuzzy: { icon: Eye, label: 'Fuzzy Location', shortLabel: 'Fuzzy', color: 'text-amber-600', bgColor: 'bg-amber-50 border-amber-200', description: 'Approximate area only' },
  social: { icon: UserCheck, label: 'Connections', shortLabel: 'Social', color: 'text-purple-600', bgColor: 'bg-purple-50 border-purple-200', description: 'Friends & connections' },
  discoverable: { icon: Globe, label: 'Discoverable', shortLabel: 'Visible', color: 'text-green-600', bgColor: 'bg-green-50 border-green-200', description: 'Open to all users' },
  beacon: { icon: Zap, label: 'Beacon Mode', shortLabel: 'Beacon', color: 'text-orange-600', bgColor: 'bg-orange-50 border-orange-200', description: 'Broadcasting location' },
};
    ...
    const fetchNotifications = async () => {
      try {
        const [messagesRes, connectionsRes] = await Promise.all([
          api.get('/api/messages').catch(() => ({ data: { conversations: [] } })),
          api.get('/api/connections/requests').catch(() => ({ data: { connections: [] } })),
        ]);
        ...
        setNotifications({ unreadMessages, pendingRequests, totalNotifications: unreadMessages + pendingRequests });
      } catch (err) {
  ...
  return (
    <>
      {(showModeSelector || showVisibilitySelector) && (
        <div className="fixed inset-0 z-[1000]" onClick={() => { setShowModeSelector(false); setShowVisibilitySelector(false); }} />
      )}

      <div className="absolute top-3 left-3 right-3 z-[1001]">
        {/* Row 1: Left controls + Right icons */}
        <div className="flex items-center justify-between gap-2">
          {/* Left: Refresh, Location, Mode Filter */}
          <div className="flex items-center gap-2">
            <button
              ...
              title="Refresh (auto-updates every 10s)"
            >
            ...
            <div className="relative">
              <button
                onClick={() => { setShowModeSelector(!showModeSelector); setShowVisibilitySelector(false); }}
                ...
                <div className="absolute top-full mt-2 left-0 z-[1002] bg-white rounded-xl shadow-xl overflow-hidden min-w-[160px]">
                  ...
                  {Object.entries(modeConfig).map(([key, cfg]) => {
                    ...
                    return (
                      <button
                        key={key}
                        onClick={() => { onModeChange(key as MingleMode); setShowModeSelector(false); }}
                        className={`w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-gray-50 transition-colors ${currentMode === key ? 'bg-gray-100 font-semibold' : ''}`}
                      >
            ...
            {/* Visibility Selector - on left side for mobile */}
            <div className="relative">
              <button
                onClick={() => { setShowVisibilitySelector(!showVisibilitySelector); setShowModeSelector(false); }}
              ...
              >
                <VisIcon size={14} />
                <span className="hidden sm:inline">{visConf.shortLabel}</span>
                ...
                <div className="absolute top-full mt-2 left-0 z-[1002] bg-white rounded-xl shadow-xl overflow-hidden min-w-[200px]">
                  ...
                  {Object.entries(visibilityConfig).map(([key, cfg]) => {
                    ...
                    return (
                      <button
                        key={key}
                        onClick={() => { onVisibilityChange(key as VisibilityLevel); setShowVisibilitySelector(false); }}
                        className={`w-full flex items-center gap-2 px-3 py-2.5 text-sm hover:bg-gray-50 transition-colors ${visibilityLevel === key ? cfg.bgColor : ''}`}
                      >
                        <Icon size={16} className={cfg.color} />
                        <div className="text-left">
                          <div className={`font-medium ${visibilityLevel === key ? cfg.color : 'text-gray-700'}`}>{cfg.label}</div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            >
              ...
              {notifications.unreadMessages > 0 && (
                <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-0.5 shadow-sm">
            ...
            >
              ...
              {notifications.pendingRequests > 0 && (
                <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] bg-purple-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-0.5 shadow-sm">
            ...
            >
              ...
              <span className="absolute bottom-0 right-0 w-2 h-2 bg-green-500 border-[1.5px] border-white rounded-full"></span>
            </button>
          </div>
        </div>
        {/* Auto-update indicator */}
        <div className="flex justify-center mt-1">
          <span className="text-[10px] text-gray-400 bg-white/60 px-2 py-0.5 rounded-full backdrop-blur-sm">
